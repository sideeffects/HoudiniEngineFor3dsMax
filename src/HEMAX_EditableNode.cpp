...
MarshallDataInto3dsMaxNURBSCVCurve(HEMAX_EditableCurve& EditableCurve, HEMAX_EditableNode& EditableNode, int PartNum)
{
    ...
    if (EditableCurve.CurveInfo.curveCount == 1)
    {
        ...
        HEMAX_Parameter ClosedParameter = GetParameter(EditableNode.Node, EditableNodeCurveClosedParm);
        std::vector<int> ClosedValue = GetParameterIntValues(ClosedParameter);
